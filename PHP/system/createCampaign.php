...
if ($connection->connect_error) {
  ...
  $query = "INSERT INTO Campaign (userID, name, partyKey, partySize) VALUES ($userID, '$campaignName', '$partyKey', 1)";
  ...
  if (!$result) {
    ...
    // We retrieve the campaignID of the newly created campaign.
    $query = "SELECT campaignID FROM Campaign WHERE userID = $userID AND name = '$campaignName' AND partyKey = '$partyKey' AND partySize = 1";

    // We execute our query.
    $result = $connection->query($query);

    // We return an error if the query does not work.
    if (!$result) {
      returnWithError($connection->connect_error);
    } else {
      ...
      // We retrieve the campaignID.
      $campaignID = $row['campaignID'];

      // We send a JSON object back with our party key and campaignID.
      returnWithInfo($campaignID, $partyKey);
...
function returnWithError($error) {
  $returnValue = '{"campaignID": 0, partyKey": "", "error": "' . $error . '"}';
...
// The returnValue variable is passed to the sendResultInfoAsJson method afterwards.
function returnWithInfo($campaignID, $partyKey) {
  $returnValue = '{"campaignID": ' . $campaignID . ', "partyKey": "' . $partyKey . '", "error": ""}';
  sendResultInfoAsJson($returnValue);
}
