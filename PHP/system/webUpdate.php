...
if ($connection->connect_error) {
  ...
  $query = "SELECT characterID, name, raceID, classID, strength, dexterity, intelligence, wisdom, constitution, charisma FROM Characters WHERE userID = $userID";
  ...
  if (!$result) {
		  ...
		  while ($rowCampaignChar = $resultCampaignChar->fetch_assoc()) {
			...
			$temp = $rowCampaignChar['campaignID'];
					...
					while($rowCampaign = $resultCampaign->fetch_assoc())
					{
						...
						array_push($raceID, $row['raceID']);
						array_push($classID, $row['classID']);
						array_push($strength, $row['strength']);
						array_push($dexterity, $row['dexterity']);
						array_push($intelligence, $row['intelligence']);
						array_push($wisdom, $row['wisdom']);
						array_push($charisma, $row['charisma']);
						array_push($constitution, $row['constitution']);
	...
	for($i = 0; $i < sizeof($DMs); $i++) {
      ...
      // We go through each campaign ID.
      for($i = 0; $i < sizeof($intelligence); $i++) {
        ...
        // If we have more than one campaign ID we want to put an comma between them each time another campaign ID appears.
          ...
          // We assign a campaign ID to $campaignIDsJSON.
          $intelligenceJSON .= '' . $intelligence[$i] . '';
      ...
      // We go through each campaign ID.
      for($i = 0; $i < sizeof($constitution); $i++) {
        ...
        // If we have more than one campaign ID we want to put an comma between them each time another campaign ID appears.
          ...
          // We assign a campaign ID to $campaignIDsJSON.
          $constitutionJSON .= '' . $constitution[$i] . '';
      ...
      returnWithInfo($characterNamesJSON, $campaignNamesJSON, $campaignIDsJSON, $DMsJSON, $partySizesJSON, $characterIDJSON, $raceIDJSON, $classIDJSON, 
						$strengthJSON, $dexterityJSON, $intelligenceJSON, $wisdomJSON, $charismaJSON, $constitutionJSON);
...
// The returnValue variable is passed to the sendResultInfoAsJson method afterwards.
function returnWithInfo($characterNames, $campaignNames, $campaignIDs, $DMNames, $partySizes, $characterID, $raceID, $classID, 
						$strength, $dexterity, $intelligence, $wisdom, $charisma, $constitution) {
  $returnValue = '{"characterNames": [' . $characterNames . '], "campaignNames": [' . $campaignNames . '], "campaignIDs": [' . $campaignIDs . '], 
					"characterID": [' . $characterID . '], "DMNames": [' . $DMNames . '], "partySizes": [' . $partySizes . '], "raceID": [' . $raceID . '],
					"classID": [' . $classID . '], "strength": [' . $strength . '], "intelligence": [' . $intelligence . '],"dexterity": [' . $dexterity . '],
